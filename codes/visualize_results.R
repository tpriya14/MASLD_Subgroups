...
run_visualization_pipeline <- function(dataset_name, input_file) {
  ...
  process_variable <- function(data, variable, subgroup = "Subgroup", adjust_method = "bonferroni") {
    ...
    kruskal_result <- kruskal.test(as.formula(paste(variable, "~", subgroup)), data = data_clean)
    ...
    p <- ggboxplot(
      data_clean, x = subgroup, y = variable, fill = subgroup,
      bxp.errorbar = TRUE, notch = TRUE, outlier.shape = 16, outlier.size = 1
    ) +
    ...
    dunn_result <- dunn_result %>% mutate(y.position = y_position_value)
    
    p <- p + stat_pvalue_manual(
      dunn_result, size = 6, label = "p.adj.signif",
      tip.length = tip_length, step.increase = step_increase_value, y.position = y_position_value
    )
    
    ggsave(paste0("results/", dataset_name, "/figures/boxplots/", variable, ".png"), plot = p, width = 6, height = 6, dpi = 400)
    ggsave(paste0("results/", dataset_name, "/figures/boxplots/", variable, ".pdf"), plot = p, width = 6, height = 6)
  ...
  disease_data <- disease_data %>% mutate(across(all_of(available_diseases), as.factor))
  
  long_data <- disease_data %>%
    pivot_longer(cols = all_of(available_diseases), names_to = "Disease", values_to = "Status") %>%
    mutate(Status = ifelse(Status %in% c("Yes", "1", 1), 1, 0))
  ...
  summary_data <- long_data %>%
    ...
    mutate(
      ...
      CI_lower = Percentage - (1.96 * SE * 100),
  ...
  ggsave(paste0("results/", dataset_name, "/figures/barplots/disease_prevalence.png"), plot = p_disease, width = 12, height = 7, dpi = 400)
  ggsave(paste0("results/", dataset_name, "/figures/barplots/disease_prevalence.pdf"), plot = p_disease, width = 12, height = 7)
  
  write.csv(summary_data, paste0("results/", dataset_name, "/statistical_results/statistical_disease_prevalence_summary.csv"), row.names = FALSE)
}
